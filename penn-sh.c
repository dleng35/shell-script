...
/* Gets the token after the redirection and redirects it to a new standard
 * out that will create if the file does not exist, truncate if there is already
 * something on the file, and write only */
void redirectionsSTDOUTtoFile (char* tokenAfter) {
...
/* Gets the token after the redirection symbol for changing the standard in
 * to the specified file. This function then redirects the standard in to the given
...
/* Creates an array of pointers (null terminated strings) before the pipe symbol
 * and returns the pointer to the start of the array */
char** createArrayOfTokensBeforePipe (char** commandArray) {
    int i = 0;
...
/* Creates an array of pointers (null terminated strings) after the pipe symbol
 * and returns the pointer to the start of the array */
char** createArrayOfTokensAfterPipe (char** commandArray) {
    int i = 0;
    int j = 0;
